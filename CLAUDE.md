...
`nomePessoa`/`documento` are the one part of `partePessoa` **not** always fake: `AtoPraticado#parte_pessoa_titulo` supplies real data when `stiposelagem == "D"` (the ato is for a título de protesto, not a regular cartório ato). In that case `id_ato` is the `protocolo` of a row in the separate legacy `cbl_tit`/`cbl_dev` domain (protest/collections module, not `sd_*`) — `CblTit` (`cbl_tit`, PK `protocolo`) holds `tipo_doc`/`cpf_cgc`, joined to `CblDev` (`cbl_dev`, PK `id_dev`, indexed on `(tipo_doc, cpf_cgc)`) which is the actual devedor (debtor) master record with the real `nome`. `tipoDocumento` is mapped from `tipo_doc` via `AtoPraticado::TIPO_DOCUMENTO_POR_TIPO_DOC` (`"CGC" => 1`, `"CPF" => 2`) — other observed `tipo_doc` values (`PF`, `CI`, `GC`) have no mapping. Endereço and the other `documento` sub-fields (`descricao`/`orgaoEmissor`/`dataEmissao`, all `minOccurs="0"` in the XSD) stay the generic placeholder regardless. Whenever the lookup can't produce confident data — not a "D" ato, no matching `cbl_tit` row, no matching `cbl_dev` row, or an unmapped `tipo_doc` — `parte_pessoa_titulo` returns `nil` and `ato_xml` falls back to the full generic placeholder rather than blocking the send (deliberate choice: submitting with the placeholder is known-safe, since that's what legacy production already does for every non-"D" ato). Not yet fired against production with a real "D" ato in the queue.
