...
**Editing `partePessoa` for retificação** — a second step, added because a correction sometimes needs to fix the party data itself (nome/documento/endereço), not just the `CGenerica` values from step one above. Two pages, not one form: submitting step one (`retificar`) already flips the ato to pending/retificação via `marcar_para_retificacao!` and redirects into step two rather than back to the dashboard.
...
- `GET/PATCH movimentacao/atos/:id/retificar/parte` (`MovimentacaoController#editar_retificacao_parte`/`#retificar_parte`) — step two's form. On `GET`, if there's no override yet, it pre-fills from whatever `parte_pessoa_dados` would send automatically (real data or nothing) as a starting point — nothing is persisted until submit. A "Pular / voltar ao painel" link skips straight back to `/movimentacao` without saving. Also reachable directly (not just via the step-one redirect) from an "Editar parte" link next to any pending ato already marked `retificacao?` on the "Atos aguardando envio" panel, so the party data can be revisited before the actual send.

**Still not tested against production** — no retification has actually been fired at TJCE yet (verified via a rolled-back transaction against a real confirmed ato in `siscartd`, not a live send). The schema also exposes a `consultaMovimentacao(idLote, idAto?)` operation (query status of an already-submitted lote/ato) that isn't used anywhere in this app either. See `docs/tjce/README.md` for details.
