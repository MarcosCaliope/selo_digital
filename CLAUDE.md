...
- `"E"` — escritura. `id_ato` is `id` of a row in `bd_escr` (yet another separate legacy domain, notary-deed module) — `BdEscr` (PK `id`, Rails default) has `gant1` ("outorgante" — grantor, the real name) and `cpfcgc_n`, same single-combined-field situation as `tblcontcertidoes.scpfcnpj` but formatted with punctuation (`.`/`-`/`/`) rather than plain digits. `bd_escr` actually has room for more parties than this branch uses: `gant1`/`gant2` are the 1st/2nd outorgante (sharing `cpfcgc_n` — concatenated when both are present, which is the source of the 22+-digit case below) and `gado1` is the 3rd person, the outorgado, with its own `cpfcgc_d`. Only the 1st party (`gant1`/`cpfcgc_n`) is used here, both because that's what this branch reads and because the TJCE XSD only allows one `<pessoa>` per ato regardless.
