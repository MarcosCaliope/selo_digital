...
One side effect surfaced by chasing this down, not yet addressed: an ato rejected by `movimentar_atos` (`status: "F"`) matches neither `pendentes_de_envio` (`lote` is no longer `0`) nor `enviados` (`status` isn't `"E"`) — it simply disappears from both dashboard panels with no visible failure reason and no built-in retry path (the TJ rejection code/message only reaches `Rails.logger.warn` and `log/soap/response_*.xml`, not any column). Recovering one today means manually resetting `status`/`lote` via console.
