...
- `"C"` — certidão. `id_ato` is `icodigo` of a row in `tblcontcertidoes` (yet another separate legacy domain) — `TblContCertidoes` (PK `icodigo`) has `snome` (real name) and `scpfcnpj`, a single column holding either a CPF or a CNPJ with no separate type column (unlike `cbl_tit`/`cbl_dev`). `tipoDocumento` is inferred from digit count of `scpfcnpj` (11 → CPF, 14 → CNPJ — the standard Brazilian convention for an untyped combined field); any other digit count, or a non-numeric value (`scpfcnpj` has legacy junk like literally `"Não Apresentou"`, and missing-leading-zero values at other lengths), falls back to `nil`/placeholder rather than guess.

Endereço and the other `documento` sub-fields (`descricao`/`orgaoEmissor`/`dataEmissao`, all `minOccurs="0"` in the XSD) stay the generic placeholder regardless of branch. Neither branch has been fired against production yet with a matching real ato in the queue.
